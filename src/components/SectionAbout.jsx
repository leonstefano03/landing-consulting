...
export default function SectionAbout({ img , href,  button, title, subTitle, parraf, bg, src, titleColor, textColor, hoverBottonTextColor }) {
  return (
    <section className={`flex w-full h-screen md:h-[700px] justify-center items-center   relative z-0 ${bg}`}>
     ...
     <div className='w-full h-full flex flex-col md:grid md:grid-cols-2 overflow-hidden'>
      <div className="w-full h-[70%] mt-14 md:my-auto md:ml-[10%]  flex text-white justify-center">
       <SectionOptions hoverBottonTextColor={hoverBottonTextColor} titleColor={titleColor} textColor={textColor} button={button} href={href} title={title} subTitle={subTitle} parraf={parraf}/>
      </div>  
      <div className=' w-full h-[30%] = md:h-full flex items-end justify-center  '>
        <Image className=' top-12 w-[50vw] sm:w-[40vw] md:w-[400px] ' width={1000 } height={0} src={src} alt='image of section about'/>
      </div></div>
        </section>
  );
}
